	...
	class NeuroNet
	{
		...
		static void Grow()
		{
			...
			1)	Анализируется количество входов в нейроны. Если где-то входов больше 10, в слой добавляется ещё один
				нейрон, после чего половина "А" переводится на новый нейрон. Выход нового нейрона должен быть в тот же
                нейрон следущиего слоя, что и тот, с которого мы сняли половину.
            2)  В случае, если ошибок слишком много, а "А" текущего нейрона распределены достаточно оптимально, то
                поднимается новый нейрон в следущем слое.
                Тут далее нужно подробнее описать критерии перевода "А" на новый нейрон.
            3)  В случае, если Текущих нейронов достаточно, поднимается вопрос о создании нового слоя. Это происходит
                В тех вариантах, когда нам нечего делить на входящем (текущем) слое, и все сигналы достаточно принимаются
                следующим (выходным) слоем.
                Тогда мы создаем новый слой, между текущим и выходным, и вставляем туда столько нейронов, сколько
                в текущем слое, и соединяем их 1-1. Далее усложняем схему по предыдущим правилам.
                (А прокатит ли оно? Или надо правила добавлениянового слоя как то связать с количеством выходных
                импульсов? Потому что количество рецпепторов и количество реакций - фиксировано).
			*/
		}
